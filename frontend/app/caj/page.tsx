    ...
    <main className="px-10 py-2">
      <div className="title-text flex font-semibold text-5xl items-center gap-4 mb-8">
        ...
        <h1 className="text-white">
          <span className="text-black opacity-50">{getLocaleStr("Pristni")}</span> {getLocaleStr("užitek")}
          <br />
          {getLocaleStr("narave")}<span className="text-black opacity-50"> {getLocaleStr("v skodelici")}</span>
        </h1>
      </div>
      <div className="caji mb-16">
        <h2 className="text-2xl text-white font-bold mb-4">ČAJI</h2>
        <div className="grid grid-cols-4 gap-x-10 gap-y-4">
          {caji.map((item) =>
            item.amount > 0 ? (
              <div key={item.name()}>
                <Link href={`/order/caj/${item.urlName}`}>
